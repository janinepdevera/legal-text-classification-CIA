...
```{r}
plotA1 <- ggplot(text %>% filter(length < 500), aes(x = length)) +
...
## III. BASELINE SCORES 
```{r}
scores_legal_w2v <- read.csv(paste0(pathdata, "/scores/sim_scores_legalw2v.csv"))
scores_glove <- read.csv(paste0(pathdata, "/scores/sim_scores_glove.csv"))
```

```{r}
scores_legal_long <- scores_legal_w2v %>% 
...
```{r}
embed_red_legal <- read.csv(paste0(pathdata, "/02 embed_dim_reduced_legal.csv"))
```

```{r}
plotC3 <- ggplot(embed_red_legal, aes(x, y)) +
...
scale_size_manual(values = c(3, 3, 3, 3, 1.3)) + 
...
```{r}
embed_red_glove <- read.csv(paste0(pathdata, "/02 embed_dim_reduced_glove.csv"))
```
